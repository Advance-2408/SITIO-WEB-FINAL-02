          ...
          {productos.map((prod, i) => (
            <div key={i} className="bg-white shadow-md rounded-xl p-4 w-[260px] md:w-[280px] flex flex-col items-center hover:scale-105 transition-transform duration-200">
              ...
              <div className="flex justify-between items-center w-full">
                ...
                <button onClick={() => agregar(prod)} className="bg-yellow-400 text-black px-3 py-1 rounded-lg font-semibold hover:bg-yellow-300 transition-all">AÑADIR</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <footer className="w-full bg-gray-900 text-white py-10 px-4 md:px-8 flex flex-wrap justify-between gap-6">
        ...
        <div className="min-w-[160px]">
          ...
          <div className="flex space-x-4 text-xl md:text-2xl">
            ...
            <a href="#" className="hover:text-orange-400">📷</a>
            <a href="#" classClassName="hover:text-orange-400">🐦</a>
            <a href="#" className="hover:text-orange-400">💬</a>
          </div>
        </div>
      </footer>
      ...
      {carritoAbierto && (
        <div className="fixed top-0 right-0 w-full sm:w-[380px] h-full bg-white shadow-2xl p-5 flex flex-col z-50">
          ...
          <div className="mt-4 border-t pt-4">
            ...
            <button className="w-full mt-4 bg-green-500 text-white py-2 rounded-lg font-semibold hover:bg-green-600 transition" onClick={() => {
              if (carrito.length === 0) return alert("Tu carrito está vacío.");
              alert("¡Gracias por tu compra! 🐾");
              setCarrito([]);
            }}>Finalizar Compra</button>

            <button className="w-full mt-2 bg-gray-300 py-2 rounded-lg hover:bg-gray-400 transition" onClick={() => setCarritoAbierto(false)}>Cerrar</button>
          </div>
        </div>
      )}
    </>
